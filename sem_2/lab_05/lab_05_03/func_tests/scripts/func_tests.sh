...
for file in $files; do
    ...
    if [ -f "$test_file" ]; then
    	./conv.exe -t2b "$file" "$test_file" # Запись тестируемых данных во входной файл
    fi
    if [ -f "$txt_file_out" ]; then
        ./conv.exe -t2b "$txt_file_out" "$file_out" #  Запись предполагаемых данных в бинарный файл
    fi
    #Проверка на наличие файлов с выходными данными и аргументами
    if { [ -f "$test_out" ] || [ -f "$file_out" ]; } && [ -f "$test_args" ]; then
        ...
        if [ -f "$file_out" ] && [ -f "$test_args" ]; then
            cmd="./func_tests/scripts/pos_case.sh $test_file $file_out $test_args f" #Команда запуска pos_case с выхлопом в определенный файл
        fi
