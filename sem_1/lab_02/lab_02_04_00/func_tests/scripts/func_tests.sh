...
for file in $files; do
    ...
    test_out="func_tests/data/neg_""$num""_out.txt" #Выходные позитивные данные

    #Проверка на наличие файла с выходными данными
    if [ -f "$test_out" ]; then
      cmd="./func_tests/scripts/neg_case.sh $file $test_out" #Команда запуска func_tests
    #При отсутствии файла
    else
        #Вывод при наличии ключа
        if [[ "$key" == "-v" ]]; then
    ...
    neg_count=$((neg_count + 1)) #Подсчет кол-ва позитивных тестов
    continue
    fi
